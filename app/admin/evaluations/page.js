...
export default function AdminEvaluationsPage() {
  ...
  // 항목 추가 폼 (템플릿별)
  const [criterionDraft, setCriterionDraft] = useState({}); // templateId -> {content, criteria_text, max_score}
  ...
  // 검토 중인 평가
  const [reviewEval, setReviewEval] = useState(null); // evaluation row
  const [reviewRows, setReviewRows] = useState([]); // [{criterion, response}]
  ...
  const saveReviewRow = async (idx, field, value) => {
  ...
  return (
    <div className="wrap">
      <div className="masthead">
        <div>
          <h1>협력업체 평가 관리</h1>
          <div className="sub">평가 템플릿 작성, 협력업체 배정, 제출자료 검토</div>
        </div>
      </div>
      ...
      <div className="panel" style={{marginBottom:20}}>
        ...
        {vendors.length === 0 && <div className="empty">아직 승인된 협력업체 계정이 없어요. 관리자 페이지에서 회원가입을 "협력업체"로 승인해주세요.</div>}
      </div>
